...
### How do I update DClaw Trademark?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp trademark --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Trademark without Kubernetes?

Yes, for local development:

```bash
cd dclaw-trademark/frontend && npm run dev
cd dclaw-trademark/backend && uvicorn dclaw_trademark.main:app --reload
```
