...
# Check events
kubectl get events -n dclaw-trademark --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-trademark deployment/dclaw-trademark-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-trademark-db-credentials -n dclaw-trademark
   ```
